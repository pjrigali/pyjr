...
from pyjr.utils.base import _check_type, _check_list, _min, _max, _mean, _variance, _std, _sum, _median, _mode, _skew, _kurtosis, _percentile
...
class PreProcess:
    ...
    def cumulative(self, stat: str = "mean", q: float = 0.75):
        ...
        if stat != "percentile":
            temp = [0.0] + [calc(data=self.cleanData.data[:i]) for i in ran]
        else:
            temp = [0.0] + [calc(data=self.cleanData.data[:i], q=q) for i in ran]
        self.data = _check_type(data=temp, dtype=self.cleanData.dtype)
        return self
    ...
    def add_constant(self, columns: int = 2):
